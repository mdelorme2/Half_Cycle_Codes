			...
			for(int k = 1; k < K; k++){
				if(khcycles[i][j][k].size() > 0 && khcycles[j][i][k].size() + khcycles[j][i][k-1].size() > 0){
					for(int l = 0; l < khcycles[i][j][k].size();l++){
						...
						/*cout << "ADD ";
						for(int m = 0; m < khcycles[i][j][k][l].size();m++){
							cout << khcycles[i][j][k][l][m] << " ";
						}
						cout << endl;*/
					}
					for(int l = 0; l < khcycles[j][i][k].size();l++){
						...
						/*cout << "ADD ";
						for(int m = 0; m < khcycles[j][i][k][l].size();m++){
							cout << khcycles[j][i][k][l][m] << " ";
						}
						cout << endl;*/
					}
					if(khcycles[i][j][k-1].size() == 0){
						for(int l = 0; l < khcycles[j][i][k-1].size();l++){
							...
							/*cout << "ADD ";
							for(int m = 0; m < khcycles[j][i][k-1][l].size();m++){
								cout << khcycles[j][i][k-1][l][m] << " ";
							}
							cout << endl;*/
						}
					}
...
/*	for(int i = 0; i < hcycles.size();i++){
	...
	//		model.addConstr(isNodeUsedM[i] + isNodeUsedE[i] <= 1);
		}
